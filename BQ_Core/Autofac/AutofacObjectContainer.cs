...
namespace BQ.Core.Autofac
{
    /// <summary>Autofac implementation of IObjectContainer.
    /// </summary>
    public class AutofacObjectContainer : IComponentContainer
    {
        ...
        public void RegisterType(Type implementationType, string serviceName = null, LifeStyle life = LifeStyle.Singleton)
        {
            ...
            if (serviceName != null)
            {
                registrationBuilder.Named(serviceName, implementationType).PropertiesAutowired();
            }
            if (life == LifeStyle.Singleton)
            {
                registrationBuilder.InstancePerDependency().PropertiesAutowired();
            }
        ...
        public void RegisterType(Type serviceType, Type implementationType, string serviceName = null, LifeStyle life = LifeStyle.Singleton)
        {
            ...
            if (serviceName != null)
            {
                registrationBuilder.Named(serviceName, serviceType).PropertiesAutowired();
            }
            if (life == LifeStyle.Singleton)
            {
                registrationBuilder.InstancePerDependency().PropertiesAutowired();
            ...
            where TImplementer : class, TService
        {
            ...
            if (serviceName != null)
            {
                registrationBuilder.Named<TService>(serviceName).PropertiesAutowired();
            }
            if (life == LifeStyle.Singleton)
            {
                registrationBuilder.InstancePerDependency().PropertiesAutowired();
        ...
        {
            var builder = new ContainerBuilder();
            var registrationBuilder = builder.RegisterInstance(instance).As<TService>().SingleInstance().PropertiesAutowired();
            if (serviceName != null)
            {
                registrationBuilder.Named<TService>(serviceName).PropertiesAutowired();
            }
